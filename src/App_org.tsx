...
function App() {
  ...
  const fetchWeatherData = useCallback(async () => {
    try {
      ...
      // プロキシ経由でAPIからデータを取得
      const response = await fetch('/api/sensor/dashboard', {
        method: 'GET',
        headers: {
          'Authorization': 'Bearer AXCI2Liuyu94PGpEl46cEa7Ck2SU0Xbv3mDc8SNi',
      ...
      const mappedData: WeatherData = {
        averageWindSpeed: data.averageWindSpeed || data.wind_speed || data.windSpeed || 0,
        averageWindSpeed10min: data.averageWindSpeed10min || data.wind_speed_10min || data.windSpeed10min || 0,
        rainIntensity10min: data.rainIntensity10min || data.rain_intensity || data.rainIntensity || 0,
        temperature: data.temperature || data.temp || data.airTemperature || 0,
        heatIndex: data.heatIndex || data.heat_index || data.wbgt || 0,
        heatIndex10min: data.heatIndex10min || data.heat_index_10min || data.wbgt10min || 0
      };
